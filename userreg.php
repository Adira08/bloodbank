  ...
  <body>
    ...
    <form method="post">
    	...
    	<div class="form-group row">
    		...
    		<div class="col-sm-9">
    			<input class="form-control"  name="dist" required="" placeholder="Distric">
    		</div>
    	</div>
    	<div class="form-group row">
    		<label for="state" class="col-sm-3 col-form-label" style="color: white;">state</label>
    		<div class="col-sm-9">
    			<input class="form-control" name="state" required="" placeholder="State">
    		</div>
    	</div>
    	<div class="form-group row">
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = "INSERT INTO users VALUES ('0','$name','$email','$phone','$group','$address','$dist','$state','$aadhar','$pass')";
        ...
        if($exec){
            $email = $_POST['email'];
            $v= mysqli_query($bloodbank,"SELECT * FROM users WHERE email='$email' ") or die (mysqli_query($bloodbank));
            ...
            ?>
            <script>
                location.replace("user/afteruserlogin.php")
              </script>
        	<?php
            exit;
